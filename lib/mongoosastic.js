...
function createEsClient (options) {
  ...
  if (Array.isArray(options.hosts)) {
    esOptions.node = options.hosts[0]
  } else {
    const host = options && options.host ? options.host : 'localhost'
    const port = options && options.port ? options.port : 9200
    const protocol = options && options.protocol ? options.protocol : 'http'
    const auth =  options && options.auth ? `${options.auth}@` : ''
    esOptions.node = `${protocol}://${auth}${host}:${port}`
    esOptions.keepAlive = false
  }

  if(options.log) {
      esOptions.log = options.log
  }
